...
<div class="postbox">
<h3 style="border-bottom: 1px solid #ddd;"><?php esc_html_e("Netsuite", 'gravity-forms-netsuite-crm') ?></h3>
<div class="inside">
...
echo '<p style="background: '.esc_attr($bg).'; padding: 10px; color: #fff " class="vx_msg_div"><i class="fa '.esc_attr($icon).'"></i> '.wp_kses_post($msg).'</p>';
  ...
  ?>
  <p>
  <button class="button" type="submit" name="<?php echo esc_attr($this->id) ?>_send" value="yes" title="<?php esc_html_e("Send to Netsuite",'gravity-forms-netsuite-crm')?>"><?php esc_html_e("Send to Netsuite",'gravity-forms-netsuite-crm')?></button>
  <?php
      if($comments ){
  ?>
    <a class="button button-secondary" title="<?php esc_html_e('Go to Logs','gravity-forms-netsuite-crm') ?>" href="<?php echo esc_url($log_url) ?>"><?php esc_html_e('Go to Logs','gravity-forms-netsuite-crm') ?></a>
  <?php
      }
  ?>
  </p>

...
if( !class_exists( 'vxg_netsuite_form' ) ) {
class vxg_netsuite_form extends vxg_netsuite{
...
$feed=get_option($this->id.'_lists',array()); 
  ...
  /**
  * Replace the Gravity Forms form choices with remotely-pulled Netsuite picklist options.
...
//var_dump($crm_fields); die();
 ...
 public function fields_html($id,$object,$refresh=false){
  ...
  if(!is_array($fields)){
      $msg= empty($fields) ? esc_html__('Fields Not Found','gravity-forms-netsuite-crm') : $fields;
  $str = '<div class="field_group_'.$this->id.'"><strong>'.__('Error:', 'gravity-forms-netsuite-crm').'</strong> '.wp_kses_post($msg).'</div>';
  ...
  */
  ...
  if(empty($value['value'])) { continue; }
        $default_html = !empty($value['default']) ?  '<strong class="default"> '.esc_html__('(Default)', 'gravity-forms-netsuite-crm').'</strong>' : '';
  ...
  */
  ...
  <tr>
  <th scope='col' class='crm_col_heading'>" . esc_html__("Pickist Field", 'gravity-forms-netsuite-crm') . "</th>
  <th scope='col' class='crm_col_heading'>" . esc_html__("Form Fields", 'gravity-forms-netsuite-crm') . "</th>
  </tr>
  </thead>
  <tbody>".$str."
  </tbody>
  ...
  $str='<div><b>'.__('No Options Found','gravity-forms-netsuite-crm').'</b></div>';    
  }
  /// $str = str_replace(array("\n", "\t", "\r"), '', str_replace("'", "\'", $str));
...
//  toggle_choices(check_box);  
  ...
  */
  ...
  $tooltips['vx_map_live_'.$this->id] = sprintf(__('%sUpdate from Netsuite %s If you update a picklist in Netsuite, the modifications will be added to your form without having to edit the field Choices in Gravity Forms. You will not be able to edit the Choices in Gravity Forms, you can only update them in Netsuite. The order of Choices as well as the default values are determined by the Netsuite picklist field settings.', 'gravity-forms-netsuite-crm'), '<h6>','</h6>');
  $tooltips['vx_map_once_'.$this->id] = sprintf(__('%sUpdate from Netsuite %s Field Choices will not be updated live from Netsuite and are editable in Gravity Forms. If you make an edit in Netsuite, it will not be updated in your form.', 'gravity-forms-netsuite-crm'), '<h6>','</h6>');
  return $tooltips;
  }
  /**
  ...
  */
  ...
  <input type="checkbox" autocomplete="off" class="<?php echo esc_attr($this->id) ?>_map_enabled crm_map_enabled" id="vx_check_<?php echo esc_attr($this->id) ?>" name="crm_map_enabled" value="<?php echo esc_attr($this->id) ?>" /> <?php esc_html_e("Enable Netsuite Field Mapping?", 'gravity-forms-netsuite-crm'); ?>
  </label>
  
  <div id="<?php echo esc_attr($this->id) ?>_map_ui" style="display: none;">
  
  <label for="<?php echo esc_attr($this->id) ?>_map_type_live">
  <input type="radio" class="<?php echo esc_attr($this->id) ?>_map_type crm_map_type" id="<?php echo esc_attr($this->id) ?>_map_type_live" name="<?php echo esc_attr($this->id) ?>_map_type" value="live" /> <?php esc_html_e("Live Remote Field Mapping ", 'gravity-forms-netsuite-crm'); gform_tooltip("vx_map_live_".$this->id); ?>
  <span class="howto" style="padding-left:1.25em;"><?php esc_html_e("Field Choices will be synced from Netsuite picklist values.", 'gravity-forms-netsuite-crm'); ?></span>
  </label>
  <label for="<?php echo esc_attr($this->id) ?>_map_type_once">
  <input type="radio" class="<?php echo esc_attr($this->id) ?>_map_type crm_map_type" id="<?php echo esc_attr($this->id) ?>_map_type_once" name="<?php echo esc_attr($this->id) ?>_map_type" value="once" /> <?php esc_html_e("Only Populate Choices ", 'gravity-forms-netsuite-crm'); gform_tooltip("vx_map_once_".$this->id) ?>
  <span class="howto" style="padding-left:1.25em;"><?php esc_html_e("Field Choices will not be updated live and are editable.", 'gravity-forms-netsuite-crm'); ?></span>
  </label>
    <label for="crm_accounts_list" style="width: 100px;" class="inline"><?php esc_html_e("Choose Account", 'gravity-forms-netsuite-crm'); ?></label>
  ...
  <option value=""><?php esc_html_e("Select a Netsuite Account", 'gravity-forms-netsuite-crm'); ?></option>
  <?php
 ...
 public function object_area($id,$object){
     ...
     $lists=array('Customer'=>'Customer');   
     ?>
       <label for="crm_object_list" style="width: 100px;" class="inline"><?php esc_html_e("Choose Object", 'gravity-forms-netsuite-crm'); ?></label>
  <?php
  if(!is_array($lists)) {
  echo esc_html__("Could not load Netsuite objects.", 'gravity-forms-netsuite-crm');
  } else { ?>
  ...
  <option value=""><?php esc_html_e("Select a Netsuite Object", 'gravity-forms-netsuite-crm'); ?></option>
  <?php

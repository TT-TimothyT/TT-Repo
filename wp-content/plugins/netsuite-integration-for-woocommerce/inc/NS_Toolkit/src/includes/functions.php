...
function arrayValuesAreEmpty( $array ) {
	...
	foreach ( $array as $key => $value ) {
		if ( false === $value || ( ! is_null( $value ) && '' != $value && ! arrayValuesAreEmpty( $value ) ) ) {
			return false;
		}
	}
...
function setFields( $object, array $fieldArray = null ) {
	...
	foreach ( $fieldArray as $fldName => $fldValue ) {
		if ( ( ( is_null( $fldValue ) || '' == $fldValue ) && false !== $fldValue ) || \arrayValuesAreEmpty( $fldValue ) ) {
			// empty param
			continue;
		}
		...
		if ( ! isset( $typesmap[ $fldName ] ) ) {
			...
			trigger_error( 'SetFields error: parameter "' . esc_html( $fldName ) . '" is not a valid parameter for an object of class "' . esc_html( $classname ) . '", it will be omitted', E_USER_WARNING );
			continue;
		}
		...
		if ( 'false' === $fldValue ) {
			...
			if ( substr( $typesmap[ $fldName ], -2 ) == '[]' ) {
				trigger_error( 'Trying to assign an object into an array parameter "' . esc_html( $fldName ) . '" of class "' . esc_html( $classname ) . '", it will be omitted', E_USER_WARNING );
				continue;
			}
			...
			if ( substr( $typesmap[ $fldName ], -2 ) != '[]' ) {
				...
				trigger_error( 'Trying to assign an array value into parameter "' . esc_html( $fldName ) . '" of class "' . esc_html( $classname ) . '", it will be omitted', E_USER_WARNING );
				continue;
			}
			...
			foreach ( $fldValue as $item ) {
				if ( is_object( $item ) ) {
					...
					$val[] = $item;
				} elseif ( 'string' == $typesmap[ $fldName ] ) {
					// handle enums
					$val[] = $item;
				} else {

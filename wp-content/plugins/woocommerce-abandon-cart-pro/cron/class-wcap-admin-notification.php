...
if ( ! class_exists( 'Wcap_Admin_Notification' ) ) {

	/**
	 * Email Verification Class
	 */
	class Wcap_Admin_Notification {
		 ...
		 * @since 9.3.0
		 */
		public static function wcap_send_admin_email( $cart_id ) {
						...
						if ( '' !== $email_id || '' !== $phone ) {
							...
							if ( $current_time >= $time_since_abandonment ) {
								...
								if ( $cart_details ) {
									...
									$send_list  = get_option( 'wcap_email_admin_custom_addresses' );
									$admin_list = self::wcap_get_send_to_list( $send_list );
									...
									$cart_link = WCAP_ADMIN_URL . '?page=woocommerce_ac_page&action=listcart';
		...
		public static function wcap_get_cart_details( $cart_id, $cart_history ) {
			...
			if ( $cart_id > 0 && $cart_history ) {
				...
				if ( isset( $cart_data ) && count( get_object_vars( $cart_data ) ) > 0 ) {
					$cart_captured = isset( $cart_data->captured_by ) ? $cart_data->captured_by : '';
				}
				$currency = isset( $cart_data->currency ) ? $cart_data->currency : '';

				$product_details = wcap_get_product_details( $cart_history->abandoned_cart_info, true );
				...
				if ( isset( $cart_history->user_id ) && $cart_history->user_id > 0 && $cart_history->user_id < 63000000 ) {
					...
					$cart_source = sprintf( __( 'Product Page, %s', 'woocomemrce-ac' ), $role_name );
...
return new Wcap_Admin_Notification();

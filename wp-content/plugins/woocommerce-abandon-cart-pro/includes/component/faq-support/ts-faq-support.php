...
class Wcap_TS_Faq_Support {
	...
	*/
	public function ts_add_new_settings_tab() {
		...
		if( isset( $_GET[ 'action' ] ) && $_GET[ 'action' ] == 'faq_support_page' ) {
		    $faq_support_page = "active";
		}
		$ts_plugins_page_url = self::$plugin_page . "&action=faq_support_page" ;
		?>
		<li class="nav-item <?php echo $faq_support_page; ?>" ><a  class="nav-link" href="<?php echo $ts_plugins_page_url; ?>" > <?php _e( 'FAQ & Support', self::$plugin_locale ); ?> </a></li>
		<?php
	...
	*/
	public function ts_faq_support_page() {
		...
		wc_get_template( '/admin/views/faq/index.php', 	
						 array(

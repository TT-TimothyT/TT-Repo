...
$rule_value_array = apply_filters( 'wcap_atc_rules_engine_rule_option_values', $rule_value_array );
$rule_data_action = apply_filters( 'wcap_atc_rules_engine_data_rule_action', '', $selected_rule_type );
...
switch ( $selected_rule_type ) {
	...
	default:
		?>
		<select id='<?php echo esc_attr( $val_id ); ?>' name='<?php echo esc_attr( $val_id . '[]' ); ?>' class="wcap_rule_value wc-product-search" multiple="multiple" data-placeholder="<?php esc_attr_e( 'Search&hellip;', 'woocommerce' ); ?>" data-action="<?php echo $rule_data_action; ?>">
			<?php
			if ( is_array( $selected_rule_value ) && count( $selected_rule_value ) > 0 ) {
				foreach ( $selected_rule_value as $v_id ) {
					if ( $v_id > 0 ) {

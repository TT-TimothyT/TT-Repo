		...
		<table class='wcap-rule-list form-table' id='wcap-rule-list' data-row='<?php echo esc_attr( $row ); ?>'>
			...
			if ( count( $rules_setup ) > 0 ) {
				...
				foreach ( $rules_setup as $rule_data ) {
					if ( 'order_status' == $rule_data->rule_type ) {
						$display_note = 'display:block;';
					}
...
<div id='wcap_show_order_status_note' style='<?php echo esc_attr( $display_note ); ?>' >
	<?php
	printf( 'Note: For emails with WC Order Status rules, the frequency will be calculated from when the order status is changed to a value matching the rule.', 'woocommerce-ac' );
	?>
</div>

...
$rule_type_options = apply_filters(
	...
	array(
		...
		'order_status'      => __( 'Order Status', 'woocommerce-ac' ),
		...
		'location_disabled' => __( 'Location', 'woocommerce-ac' ),
		'countries'         => __( 'Countries', 'woocommerce-ac' ),
	)
);
...
foreach ( $rule_type_options as $type_key => $type_value ) {
	...
	if ( in_array( $type_key, array( 'cart_status', 'payment_gateways', 'order_status', 'product_cat', 'product_tag', 'cart_items', 'cart_items_count', 'cart_total', 'countries' ) ) ) {
...
switch ( $selected_rule_type ) {
	...
	case 'countries':
		?>
		<select id='<?php echo esc_attr( $val_id ); ?>' name='<?php echo esc_attr( $val_id . '[]' ); ?>' class="wcap_rule_value wc-enhanced-select" multiple="multiple">
			<?php
			foreach ( $rule_value_array as $code => $name ) {
				$selected = '';
				if ( in_array( $code, $selected_rule_value ) ) {
					$selected = 'selected';
				}
				echo '<option value="' . esc_attr( $code ) . '"' . $selected . '>' . wp_kses_post( $name ) . '</option>';
			}
			?>
		</select>
		<?php
	...
	case 'order_status':
		?>
		<select id='<?php echo esc_attr( $val_id ); ?>' name='<?php echo esc_attr( $val_id . '[]' ); ?>' class="wcap_rule_value wc-product-search" multiple="multiple" data-placeholder="<?php esc_attr_e( 'Search for a Status&hellip;', 'woocommerce' ); ?>" data-action="wcap_json_find_order_status">
			<?php
			foreach ( $selected_rule_value as $v_id ) {
				if ( '' !== $v_id ) {
					$v_object = $rule_value_array[ $v_id ];

...
?>
<p>
	<?php echo esc_html__( 'Hi', 'woocommerce-ac' ); ?>,
</p>
<div>
	<p>
		<?php
		echo sprintf(
			...
			esc_html__( 'We are writing to let you know that a cart has been abandoned on %s', 'woocommerce-ac' ),
			esc_html( $blog_name )
		);
		?>
	</p>
	...
	<p>
		<strong>
			...
			echo sprintf(
				...
				esc_html__( 'Cart #%1$s (%2$s)', 'woocommerce-ac' ),
		...
		echo sprintf(
			...
			esc_html__( 'The cart was captured by %1$s', 'woocommerce-ac' ),
...
<div style="margin-bottom: 40px;">
	<table class="td" cellspacing="0" cellpadding="6" style="width: 100%; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;" border="1">
		<thead>
			<tr>
				<th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;"><?php esc_html_e( 'Product', 'woocommerce-ac' ); ?></th>
				<th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;"><?php esc_html_e( 'Quantity', 'woocommerce-ac' ); ?></th>
				<th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;"><?php esc_html_e( 'Price', 'woocommerce-ac' ); ?></th>
			</tr>
		</thead>
		<tbody>
			<?php
			...
			foreach ( $cart_details->product_data as $details ) {
				...
				<tr>
					...
					<td class="td" style="text-align:<?php echo esc_attr( $text_align ); ?>; vertical-align:middle; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;">
						<?php echo wp_kses_post( apply_filters( 'acfac_change_currency', Wcap_Common::wcap_get_price( $details->line_subtotal, $cart_details->currency ), $cart_details->id, $details->line_subtotal, 'wcap_cron' ) ); ?>
					</td>
				</tr>
				<?php
			}
		...
		<tfoot>
			...
			if ( is_array( $item_totals ) && count( $item_totals ) > 0 ) {
				...
				foreach ( $item_totals as $total ) {
					$price = apply_filters( 'acfac_change_currency', Wcap_Common::wcap_get_price( $total['value'], $cart_details->currency ), $cart_details->id, $total['value'], 'wcap_cron' );
					$i++;
					?>
					<tr>
...
<div style="font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif; margin-bottom: 40px;">
	<p>
		<strong><i>
			<?php
			echo esc_html__( 'Customer Details', 'woocommerce-ac' );
			?>
		</i></strong>
	</p>
	<address>
		...
		<?php
		// Translators: Phone.
		echo esc_html( sprintf( __( 'Phone: %s', 'woocommerce-ac' ), $customer_details->phone ) );
		?>
		<br>
		<?php
		// Translators: Email.
		echo esc_html( sprintf( __( 'Email: %s', 'woocommerce-ac' ), $customer_details->email ) );
		?>
	</address>
...
<div>
	<p>
		...
		echo wp_kses_post( sprintf( __( 'You can check the cart details %s', 'woocommerce-ac' ), '<a href="' . $cart_link . '" target="_blank">' . __( 'here', 'woocommerce-ac' ) . '</a>' ) );
		?>
</div>

<?php
/**
 * Abstract Class for Calls. All calls across connectors inherit this class.
 *
 * @since 8.15.0
 */

abstract class Wcap_Call {
	...
	public function get_random_api_error() {
		return __( 'Api Error: No response from API', ' woocommerce-ac' );
	}
	...
	public function set_data( $data ) {
		$data       = apply_filters( 'wcap_modify_set_data', $data );
		$this->data = $data;
	}
	...
	public function make_wp_requests( $url, $params = array(), $headers = array(), $req_method = 1 ) {
		$body = array(
			'response' => 500,
			'body'     => array( __( 'CURL Error', 'woocommerce-ac' ) ),
		);

...
if ( !class_exists('Wcap_Order_Received' ) ) {
    ...
    class Wcap_Order_Received {
        ...
        public static function  wcap_action_after_delivery_session( $order ) {
            ...
            $wcap_order = wc_get_order( $order_id );
            $wcap_get_order_status = $wcap_order->get_status();
            
            $get_abandoned_id_of_order  = $wcap_order->get_meta( 'wcap_recover_order_placed', true );
            $get_sent_email_id_of_order = $wcap_order->get_meta( 'wcap_recover_order_placed_sent_id', true );
				...
				if ( is_array( $coupons_meta ) && count( $coupons_meta ) > 0 ) {
					foreach ( $coupons_meta as $key => $coupon_details ) {
						...
						if ( is_array( $coupon_details ) && isset( $coupon_details['atc_coupon_code'] ) && '' !== $coupon_details['atc_coupon_code'] && ! $details_added ) {
							...
							$wcap_order->update_meta_data( 'wcap_atc_coupon', $coupon_code );
                            $wcap_order->add_order_note( "ATC Coupon $coupon_code was used.", 'woocommerce-ac' );
                ...
                if ( isset( $get_abandoned_id_of_order ) && '' != $get_abandoned_id_of_order ){
                  ...
                  if ( count( $get_abandoned_cart_user_id_results ) > 0 ) {
                    ...
                    if ( $wcap_user_id >= 63000000 ) {
					    WCAP_GUEST_CART_MODEL::delete( array( 'id' => $wcap_user_id ) );                     
                    }

                        WCAP_CART_HISTORY_MODEL::delete( array( 'id' => $get_abandoned_id_of_order ) );
                        $order = wc_get_order( $order_id );
						if ( $order ) {
							$order->delete_meta_data( 'wcap_recover_order_placed' );
							$order->save();
						}
                    }
                }

...
class Wcap_Webhooks {
	...
	public static function wcap_generate_payload( $payload, $resource, $resource_data, $id ) {
		if ( is_array( $resource_data ) && count( $resource_data ) > 0 && isset( $resource_data['action'] ) ) {
	...
	public static function wcap_atc_created( $abandoned_cart_id ) {
		...
		$cart_history_data = wcap_get_data_cart_history( $abandoned_cart_id, $force_check = true );

		if ( isset( $cart_history_data['cart_history'] ) && ! empty( isset( $cart_history_data['cart_history'] ) ) && isset( $cart_history_data['guest_data'] ) && ! empty( isset( $cart_history_data['guest_data'] ) ) ) {
			// Prepare the Data to be sent.
			$cart_value = json_decode( stripslashes( $cart_history_data['cart_history']->abandoned_cart_info ) );
			...
			$send_data = array(
				'id'           => $cart_history_data['cart_history']->id,
				'product_id'   => $product_id,
				'variation_id' => $variation_id,
				'product_name' => $product_name,
				'timestamp'    => $cart_history_data['cart_history']->abandoned_cart_time,
				'email_id'     => $cart_history_data['guest_data']->email_id,
				'phone'        => $cart_history_data['guest_data']->phone,
				'source'       => 'atc',
			);
			...
			$data = apply_filters( 'wcap_popup_record_created', $data );
		}
	...
	public static function wcap_guest_forms( $abandoned_cart_id ) {
		...
		if ( isset( $cart_history_data['cart_history'] ) && ! empty( isset( $cart_history_data['cart_history'] ) ) && isset( $cart_history_data['guest_data'] ) && ! empty( isset( $cart_history_data['guest_data'] ) ) ) {
			// Prepare the Data to be sent.
			$product_details = wcap_get_product_details( $cart_history_data['cart_history']->abandoned_cart_info );
			...
			$data = apply_filters( 'wcap_guest_record_via_form', $data );
		}
	...
	public static function wcap_checkout_created( $abandoned_cart_id ) {
		...
		if ( isset( $cart_history_data['cart_history'] ) && ! empty( isset( $cart_history_data['cart_history'] ) ) && isset( $cart_history_data['guest_data'] ) && ! empty( isset( $cart_history_data['guest_data'] ) ) ) {
			...
			$send_data = array(
				...
				'product_details'    => $product_details,
				'timestamp'          => $cart_history_data['cart_history']->abandoned_cart_time,
				'billing_first_name' => $cart_history_data['guest_data']->billing_first_name,
				'billing_last_name'  => $cart_history_data['guest_data']->billing_last_name,
				'billing_country'    => $cart_history_data['guest_data']->billing_country,
				'billing_zipcode'    => $cart_history_data['guest_data']->billing_zipcode,
				'email_id'           => $cart_history_data['guest_data']->email_id,
				'phone'              => $cart_history_data['guest_data']->phone,
				'source'             => 'checkout',
			);
			...
			$data = apply_filters( 'wcap_checkout_record_created', $data );
	...
	public static function wcap_reminder_emails( $abandoned_id = 0, $sent_id = 0, $links = array() ) {
		...
		if ( $abandoned_id > 0 && $sent_id > 0 ) {
			...
			if ( is_array( $send_data ) ) {
				...
				$data = apply_filters( 'wcap_reminder_sent_email', $data );
			}
		}
	}
	...
	public static function wcap_reminder_sms( $abandoned_id = 0, $template_id = 0 ) {
		...
		if ( $abandoned_id > 0 && $template_id > 0 ) {
			...
			if ( is_array( $send_data ) ) {
				...
				$data = apply_filters( 'wcap_reminder_sent_sms', $data );
			}
		}
	}
	...
	public static function wcap_reminder_fb( $abandoned_id = 0, $template_id = 0 ) {
		...
		if ( $abandoned_id > 0 && $template_id > 0 ) {
			...
			if ( is_array( $send_data ) ) {
				...
				$data = apply_filters( 'wcap_reminder_sent_fb', $data );
			}
		}
	}
	...
	public static function wcap_recovery_link( $cart_id, $recovery_medium, $link_used ) {
		...
		if ( $cart_id > 0 ) {
			...
			if ( is_array( $send_data ) ) {
				...
				$data = apply_filters( 'wcap_recovery_link', $data );
			}
		}
	}
	...
	public static function wcap_cart_recovered( $abandoned_id, $order_id ) {
		...
		if ( $abandoned_id > 0 && $order_id > 0 ) {
			...
			if ( is_array( $send_data ) ) {
				...
				$data = apply_filters( 'wcap_abandoned_cart_recovered', $data );
			}
		}
	}
	...
	public static function wcap_cart_cutoff_reached( $abandoned_cart_id ) {
		if ( $abandoned_cart_id > 0 ) {
			...
			if ( $cart_data ) {
				...
				$data = apply_filters( 'wcap_abandoned_cart_cutoff', $data );
			}
		}
	}
}

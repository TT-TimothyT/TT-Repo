...
if ( !class_exists('Wcap_Tiny_Mce' ) ) {
    ...
    class Wcap_Tiny_Mce{
        ...
        public static function wcap_format_tiny_MCE( $in ) {
            ...
            $in['remove_redundant_brs']         = false; // Do not remove redundant BR tags.
            $in['valid_elements']               = '*[*]';
            $in['valid_children']               = "+html,+head,+body,+meta,+title";
            $in['extended_valid_elements']      = "html[*],head,meta[*],title[*],body,script[*],style[*],link[*]";
            $in['custom_elements']              = "html,head,body,script,style,link,meta,title";
            $in['plugins']                      = 'fullscreen';
        ...
        public static function wcap_filter_mce_button( $buttons ) {
			$action = isset( $_GET['action'] ) && '' !== $_GET['action'] ? sanitize_text_field( $_GET['action'] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification

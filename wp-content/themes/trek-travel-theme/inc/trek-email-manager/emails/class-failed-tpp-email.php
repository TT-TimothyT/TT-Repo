 ...
 * Failed TPP Email
 *
 * An email sent to the admin when a Travel Protection Purchase fails in NetSuite.
 *
 * @class       Failed_TPP_Email
 * @extends     WC_Email
 *
 */

class Failed_TPP_Email extends WC_Email {
	
	function __construct() {
		
		// Add email ID, title, description, heading, subject.
		$this->id                   = 'failed_tpp_email';
		$this->title                = __( 'Failed TPP Email', 'trek-travel-theme' );
		$this->description          = __( 'This email is received when a Travel Protection Purchase fails in NetSuite.', 'trek-travel-theme' );
		
		$this->heading              = __( 'Failed TPP Email', 'trek-travel-theme' );
		$this->subject              = __( '[{blogname}] TPP Failed in NetSuite: Booking ID - {booking_id} (Order {order_number}) - {order_date}', 'trek-travel-theme' );

		// Email template path.
		$this->template_html    = 'emails/failed-tpp-email.php';
		$this->template_plain   = 'emails/plain/failed-tpp-email.php';

		// Triggers for this email.
		add_action( 'failed_tpp_pending_email_notification', array( $this, 'queue_notification' ), 10, 2 );
		add_action( 'failed_tpp_email_notification', array( $this, 'trigger' ), 10, 3 );
	...
	public function queue_notification( $order_id, $ns_response ) {
		...
		foreach ( $items as $item_key => $item_value ) {
			...
			if ( in_array( $product_id, $accepted_p_ids ) ) {
				// This is the Travel Protection product.
				$item_id    = $item_key;
		...
		wp_schedule_single_event( time(), 'failed_tpp_email', array( $item_id, $order_item, $ns_response ) );
	}
	...
	function trigger( $item_id, $order_item, $ns_response ) {
		...
		if ( $item_id && $send_email ) {
			...
			$key = array_search( '{booking_id}', array_keys( $this->placeholders ) );
			if ( false !== $key ) {
				unset( $this->placeholders[ $key ] );
			}

			$this->placeholders = array_merge( array( '{booking_id}' => $this->object->booking_id ), $this->placeholders );
	...
	public static function create_object( $item_id, $order_item, $ns_response = array() ) {
		...
		// Trek checkout data
		$trek_protection_data = wc_get_order_item_meta( $item_id, 'tt_protection_data', true );
		$booking_order_id     = isset( $trek_protection_data['order_id'] ) ? $trek_protection_data['order_id'] : 0;
		$booking_id           = get_post_meta( $booking_order_id, 'tt_meta_guest_booking_id', true );
		$cart_total           = $order->get_total( $order_item );

		// Related Booking Order ID.
		$email_data_object->booking_order_id = $booking_order_id;

		// Booking ID.
		$email_data_object->booking_id = $booking_id ? $booking_id : 0;
